...
```bash
node skills/fluid/detector/detect.mjs <path>     # audit a folder or file
node skills/fluid/detector/detect.mjs --strict <path>   # exit 1 on warnings (CI)
node skills/fluid/detector/detect.mjs --json <path>     # machine output
```
...
`skills/examples/bitcoin/` is a complete design system generated by the `design-system` skill: orange `#f7931a` + mint + gold, Cabinet Grotesk + Satoshi + JetBrains Mono, dark-first. Open `storybook.html` to see it. It passes the `fluid` detector clean.
